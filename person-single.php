  ...
  <div class="wrapper">
    ...
    <div class="container rounded bg-white mt-5 mb-5">
      ...
      <?php
          ...
          function add_relatives_nodes($node_id){
            $nodes = [];
            $edges = [];
            global $query, $db, $displayed_ids;
            $displayed_ids_string = implode(',', $displayed_ids);
          ...
          $network_data = add_relatives_nodes($id);
          $nodes = array_merge($nodes, $network_data[0]);
          $edges = array_merge($edges, $network_data[1]);
        ?>
      ...
      <script>
        ...
        //clear the arrays for further use
          ...
          if (chosen_node != undefined) {
            ...
            $.ajax({
              ...
              success: function(response) {
                ...
                var arr = <?php echo json_encode($nodes, JSON_UNESCAPED_UNICODE) ?>;
                alert(arr);

                var nodes = JSON.parse(response.nodes);
                var edges = JSON.parse(response.edges);
                alert(nodes);
              },
              error: function() {
